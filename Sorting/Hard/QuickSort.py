...
# Space complexity O(nO)
from random import randint

def quickSort_extraSpace(array):
    if len(array) <= 1:
        return array
    elif len(array) == 2:
        return array if array[0]<=array[1] else array[::-1]
    
    rIdx = randint(0, len(array)-1)
    le = [value for idx, value in enumerate(array) if value<=array[rIdx] and idx!=rIdx]
    gt = [value for idx, value in enumerate(array) if value>array[rIdx] and idx!=rIdx]

    return quickSort_extraSpace(le) + [array[rIdx]] + quickSort_extraSpace(gt)
...
def test_quickSort(array, expected):
    assert quickSort(array)==expected
